...
function App() {
  ...
  useEffect(() => {
    const cleanupAndMigrate = async () => {
      ...
      // Only migrate campaigns where requiresReport is not explicitly false
      const completedEvents = events.filter(e => e.status === 'Tamamlandı' && e.requiresReport !== false);
      ...
      for (const event of completedEvents) {
        ...
        const reportDueDate = calculateReportDueDate(completionDate);

        try {
          await addDoc(collection(db, "reports"), {
            title: `${event.title} Raporu`,
            campaignId: event.id,
          ...
          console.log(`Migrated: ${event.title} -> Report due ${format(reportDueDate, 'd MMMM yyyy', { locale: tr })}`);
        } catch (error) {
          console.error(`Migration failed for ${event.title}:`, error);
        }
      }
    };
