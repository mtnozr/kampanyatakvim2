...
function App() {
  ...
  // Handle marking a campaign report as done
  const handleMarkReportDone = async (eventId: string) => {
    ...
    try {
      await updateDoc(doc(db, "events", eventId), {
        reportStatus: 'done',
        reportCompletedAt: Timestamp.now(),
        reportCompletedBy: reporterId,
        updatedAt: Timestamp.now()
      });
      ...
      // Log the action
      await addDoc(collection(db, "logs"), {
        message: `Kampanya raporu tamamlandı: ${event.title} (ID: ${eventId.substring(0, 6).toUpperCase()})`,
        timestamp: Timestamp.now()
      });
    } catch (e) {
  ...
  // Handle updating report due date (drag-drop)
  const handleUpdateReportDueDate = async (eventId: string, newDate: Date) => {
    if (!isDesigner) return;
    ...
    try {
      await updateDoc(doc(db, "events", eventId), {
        reportDueDate: Timestamp.fromDate(newDate),
        updatedAt: Timestamp.now()
      });
      ...
      // Log the action
      await addDoc(collection(db, "logs"), {
        message: `Rapor tarihi değiştirildi: ${event.title} -> ${format(newDate, 'd MMMM yyyy', { locale: tr })}`,
        timestamp: Timestamp.now()
      });
    } catch (e) {
  ...
  // Handle day click in report calendar (opens add modal with report context)
        ...
        <div id="printable-calendar" className="p-1">
          ...
          <div className="flex items-center gap-2 mb-6">
            ...
            {canSeeReportTab && (
              ...
              >
                📊 RAPOR
                {events.filter(e => e.status === 'Tamamlandı' && e.reportStatus !== 'done').length > 0 && (
                  <span className="absolute -top-1 -right-1 w-5 h-5 bg-amber-500 text-white text-[10px] font-bold flex items-center justify-center rounded-full border-2 border-white dark:border-slate-800">
                    {events.filter(e => e.status === 'Tamamlandı' && e.reportStatus !== 'done').length}
                  </span>
                )}
              </button>
            )}
